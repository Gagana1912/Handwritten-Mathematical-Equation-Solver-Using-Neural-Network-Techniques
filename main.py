...
class MainWindow(QWidget):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("HANDWRITTEN MATHEMATICAL EQUATION SOLVER USING NEURAL NETWORK TECHNIQUES")
        self.showMaximized()
    ...
    def create_widgets(self):
        ...
        self.lbl_image.setText("DRAG AND DROP AN IMAGE HERE")
        self.lbl_image.setAlignment(Qt.AlignCenter)

        self.btn_open_image = QPushButton("OPEN")#is a simple button in PyQt, when clicked by a user some associated action gets performed
    ...
    def solve_equation(self):
        pixmap = self.lbl_image.pixmap()#Pixmap class is an off screen image representation that can be used as paint device
        if pixmap:
    ...
    def process_expression(self, image):#solves simple expressions like 4-3,8+3
        try:
    ...
    def process_quadratic_eq(self, image):#solves quadratic equation
        try:
    ...
    def process_cubic_eq(self, image):#solves cubic equation
        try:
            equation = solver.extract(image)
            equation, solution = solver.solve_cubic_equation(equation)
    ...
    def process_linear_system(self, image):#solves linear equations
        try:
    ...
    def pixmap_to_numpy(pixmap):
        ...
        ptr = image.bits()#returns binary representation for the specified byte position 
